...
$sql = "SELECT tr.*, f.name AS trail_name, u.username
        FROM trail_reports tr 
        INNER JOIN features f ON tr.feature_id = f.id
        INNER JOIN users u ON tr.user_id = u.user_id WHERE tr.active = 0";
...
<div class="trail-reports">
    ...
    if (mysqli_num_rows($result) === 0) {
        ...
        while ($report = mysqli_fetch_assoc($result)) {
            ...
            echo "  <h5><a href='./trail_report.php?id=" . $report['id'] . "'>" . htmlspecialchars($report['title'], ENT_QUOTES, 'UTF-8') . "</a></h5>";
